  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">曲风流派</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">押韵难度</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">嘻哈歌词写字与 Flow 运腔指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 说唱嘻哈歌词与双押专家');
...
const presetOptions = [
  "BoomBap 老学校传统 16 Bars 街头写实",
  "Trap 快节奏甩拍与三连音双押歌词",
  "R&B 旋律说唱 (Melodic Rap) 甜爱情歌",
  "硬核战歌 (Hardcore) Battle 炸裂 Punchline"
];
...
const metricsList = [
  {
    "key": "rhymeMultiPatternRigor",
    "label": "双押/三押韵脚严谨度"
  },
  {
    "key": "flowRhythmDelivery",
    "label": "Flow 节奏运腔顺畅度"
  },
  {
    "key": "hookMelodyCatchiness",
    "label": "Hook 记忆点洗脑分"
  },
  {
    "key": "punchlineImpactIndex",
    "label": "Punchline 爆点打击力"
  },
  {
    "key": "hiphopAttitudeAuthenticity",
    "label": "嘻哈态度真实度"
  }
];
const guideRules = [
  {
    "title": "押韵自然不硬凑",
    "advice": "双押三押要服务于主题与态度表达，自然流畅，避免为了押韵使用无关词汇",
    "avoid": "切忌为了双押而凑出毫无逻辑逻辑的搞笑词汇"
  },
  {
    "title": "Punchline 一击必杀",
    "advice": "在 Verse 结尾使用强有力的比喻或谐音双关（Wordplay），引爆听众耳朵",
    "avoid": "避免整段歌词平铺直叙缺乏记忆爆点"
  },
  {
    "title": "Flow 卡拍留白",
    "advice": "歌词字数要配合拍子节奏，适当在每小节第 4 拍留白喘息，增加运腔律动",
    "avoid": "严禁塞入过多文字导致吐字不清含糊不清"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[SHUOCHANG_SCORES\][\s\S]*?\[\/SHUOCHANG_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[SHUOCHANG_SCORES\](.*?)\[\/SHUOCHANG_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('shuochang_history_records');
...
const saveHistory = () => {
  localStorage.setItem('shuochang_history_records', JSON.stringify(historyList.value));
};
